...
class player_ui(YoutubePlayer):
    def __init__(self):
        # Update this variable as True only when a new list has been initlaized
    ...
    def make_player_ui(self):
        ...
        self.txt2_4 = urwid.Text(
            f"Volume: {(vol-1)*'█'}{(9-vol)*'░'}", align='right')
...
class my_Text(urwid.Text):
    ...
    def keypress(self, size, key):
        # CAN't FIND ANY OTHER WAY THAN THIS TO CHANGE THE VARIABLES OF MY player_ui CLASS
        global LIST_LOCK
        if(key == 'enter'):
            LIST_LOCK = False
        elif(key == 'q'):
